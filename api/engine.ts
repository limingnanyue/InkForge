...
function detectExpiredForeshadows(foreshadowing: Foreshadow[], currentChapterIdx: number): Foreshadow[] {
  return foreshadowing.map(f => {
    ...
    if (currentChapterIdx - f.plantedAt > tolerance) {
      // 风险1修复: 高优先级主线伏笔永不自动过期，只标超期提醒（防长篇主线伏笔被误判丢失）
      // 保持 planted，buildDynamicContext 会标【已超期·强烈建议回收·主线伏笔不可丢失】
      // 原: high 伏笔超 30 章容忍期也判 expired → 不可再埋 → 第120章无法回收 → 主线崩塌
      if (f.importance === 'high') return f;
...
export function buildDynamicContext(projectId: string, currentChapterIdx?: number): string {
  ...
  if (pending.length > 0) {
    sections.push(`【待回收伏笔 · 务必在合适章节回收】\n${pending.map((f, i) => {
      ...
      const tolerance = f.importance === 'high' ? FORESHADOW_EXPIRY_HIGH : FORESHADOW_EXPIRY_NORMAL;
      const overdue = age > tolerance - 3 ? '【已埋N章·优先回收】' : '';
      // 风险1修复: 高优先级主线伏笔超 tolerance（detectExpiredForeshadows 已保持 planted 不改 expired）
      //   → 标【已超期·强烈建议回收·主线伏笔不可丢失】硬提醒，防长篇主线伏笔被误判丢失后无法回收
      const highOverdue = f.importance === 'high' && age > tolerance ? '【已超期·强烈建议回收·主线伏笔不可丢失】' : '';
      ...
      return `${i + 1}. 第${f.plantedAt + 1}章埋设 ${imp}${overdue}${highOverdue}${dueNow}：${f.desc}${expect}`;
    }).join('\n')}`);
  }
  ...
  if (state.conflictLines && state.conflictLines.length > 0) {
    ...
    // 风险2修复: active 按 level 优先级（book>volume>chapter）+ introducedAt 降序排序后再 slice(0,10)
    //   原: active 按插入顺序 slice(0,10) 取最早 10 个,后期埋的书级矛盾 push 到末尾被截断 → LLM 看不到书级主线矛盾
    //   现: 书级矛盾必入前 10,章级小矛盾靠后,保证长篇中后段书级主线矛盾不被截断
    const levelOrder: Record<ConflictLine['level'], number> = { book: 0, volume: 1, chapter: 2 };
    const active = cls.filter(c => c.status === 'active')
      .sort((a, b) => {
        const lo = levelOrder[a.level] ?? 3;
        const ro = levelOrder[b.level] ?? 3;
        if (lo !== ro) return lo - ro;  // book 优先
        return (b.introducedAt ?? 0) - (a.introducedAt ?? 0);  // 同 level 新的优先
      });
...
export function verifyMemoCompliance(content: string, memo: ChapterMemo): { passed: boolean; missing: string[] } {
  ...
  if (content.length > 500 && memo.hook) {
    ...
    // 收紧信号词:移除'！'/'突然'/'只见'/'然而'(AI味词)/'倒底'(错别字)/'到底'等可绕过项,保留真悬念词(与节奏门保持一致)
    const hookSignals = ['？', '...', '……', '不料', '未完', '究竟', '难道', '谁知', '何曾', '岂料', '忽见', '此刻'];
    const hasSignal = hookSignals.some(s => tail200.includes(s));
    // 增补悬念结构检测:章末 200 字含疑问句(？)或反转信号(但/却/不料+后接新信息)
    const hasReversal = /(但|却|不料)[^，。！？\n]{2,30}/.test(tail200);
    const hasHook = hasSignal || hasReversal;
    if (!hasHook) {
      missing.push(`章末钩子缺失:memo 钩子「${memo.hook.slice(0, 40)}」未在章末 200 字检测到悬念信号词(？/省略号/不料等)或反转结构(但/却/不料+新信息)`);
    }
  }
...
· 核心情绪：按题材对位（见题材→情绪路由表）`;
  ...
  // 风险4修复: 全书核心锚点优先用 state.mainlineProgress（每10章审稿增量更新,含主线演进信息）
  //   降级用 state.idea 前200字（旧数据或未到审稿周期）
  //   原: 仅 idea 前200字,200章长篇只描述起点创意,不含主线演进（已解锁伏笔/已转折冲突/已建立关系）
  const _projState = stateRepo.get(projectId);
  const mainlineSrc = _projState?.mainlineProgress?.trim() || _projState?.idea || '';
  const ideaAnchor = mainlineSrc
    ? `\n【全书核心锚点】${mainlineSrc.slice(0, 200)}${mainlineSrc.length > 200 ? '…' : ''}
...
文风要求：流畅、有画面感、节奏紧凑。章末留钩，驱动翻页。

【去AI味硬约束·生成端预防】（精简版 7 Gate 禁用清单前置到生成端,生成即合规,降低精修负担）
- 禁用副词：仿佛/宛如/犹如/似乎/不禁/不由得/淡淡的/缓缓的/微微的/轻轻的
- 禁用翻转句：不是A而是B / 并非A而是B / 与其说不如说
- 禁用递进句：不仅A而且B / 不仅A还B
- 禁用解释腔：然而/值得注意的是/总而言之/综上所述/众所周知/不难发现
- 禁用升华句：这便是X / 从此他明白了Y / 那一刻他懂了
- 禁用上帝视角：之所以X是因为Y / 殊不知 / 多年以后 / 他不知道的是
- 禁用排比三连（三句相同句式连用）
- 对话要口语化，符合人物身份/年龄/知识盲区，禁书面过渡词（然而/不仅如此/由此可见）
- 心理活动外化为动作/微表情（"他攥拳"而非"他感到愤怒"）`,
...
export interface GenerateParams {
  ...
  // 风险3修复: 透传单次调用总耗时上限(ms)给 streamComplete,缺省走 WALL_TIMEOUT_MS(5分钟)。
  //   章节正文生成按字数动态放宽(每1000token给100秒,至少8分钟),防国产慢模型流式输出超5min被误杀
...
export async function* runSkill(params: GenerateParams): AsyncGenerator<string> {
  ...
  yield* streamComplete({
    ...
    wallTimeoutMs: params.wallTimeoutMs,
  });
}
...
// 国产模型(deepseek/qwen/glm 等)中文 1 字≈1.5-2 token,OpenAI 系≈1.2,未知保守 1.5
// 风险2修复: CN_KINDS ratio 1.5→1.8(取中值偏上)。实测国产模型可达 2.0,
//   原 1.5 时 3000 字需 6000 token 只给 4500 → 章节结尾截断;1.8 平衡截断风险与 token 成本
//   (与 daemon.ts 同步,两处必须一致)
...
export async function checkChapterQuality(opts: {
  ...
  if (actualLen >= minHard) {
    ...
    const AI_TASTE_PATTERNS = [
      // 原有副词
      /仿佛/g, /宛如/g, /犹如/g, /似乎/g, /不禁/g, /不由得/g,
      // 的/地双形态副词(原只检测"的",补"地"漏检)
      /淡淡[的地]/g, /缓缓[的地]/g, /微微[的地]/g, /轻轻[的地]/g,
      // 翻转句式
      /不是.{0,30}而是/g, /并非.{0,30}而是/g, /这便是.{0,10}/g, /与其说.{0,30}不如说/g,
      // 递进句式(新增)
      /不仅.{0,30}(而且|还|更)/g,
      // 解释过渡词(新增,AI 解释腔)
      /然而[^，。]/g, /值得注意的是/g, /总而言之/g, /综上所述/g, /众所周知/g, /不难发现/g, /与此同时/g, /由此可见/g,
      // 升华句(补齐)
      /那一刻.{0,10}(懂|明白)/g, /(他|她)明白了/g, /(他|她)终于明白/g,
      // 总结句
      /本章[^。]*。/g, /至此[^。]*。/g, /就这样[^。]*。/g, /从此[^。]*。/g,
      // 上帝视角
      /之所以.{0,30}是因为/g, /殊不知/g, /多年以后/g, /(他|她)不知道的是/g,
      // 排比三连检测(三句相同句式:X了,X了,X了;兑现注释承诺)
      /(.{2,8})了[，。].*\1了[，。].*\1了/g,
    ];
    let aiTasteHits = 0;
    ...
    // 命中数 > 4 视为 AI 味重,加 issue 触发重写(5-8 hits 也产生 issue,原 >8 门槛断层已修)
    const aiTasteScore = aiTasteHits > 12 ? 0.3 : (aiTasteHits > 8 ? 0.5 : (aiTasteHits > 4 ? 0.7 : 0.9));
    // 短章按字数归一化:密度 > 3/千字也触发 issue(防短章命中少但比例高)
    const aiTasteDensity = aiTasteHits / Math.max(actualLen, 1) * 1000;
    if (aiTasteHits > 4 || aiTasteDensity > 3) {
      issues.push(`AI 味重：命中 ${aiTasteHits} 个禁用模式（密度 ${aiTasteDensity.toFixed(1)}/千字）（${hitPatterns.slice(0, 3).join(' / ')}${hitPatterns.length > 3 ? '...' : ''}）`);
    }
    ...
    if (actualLen > 500) {
      const tail200 = content.slice(-200);
      // 收紧信号词:移除'！'/'突然'/'只见'/'然而'(AI味词)/'倒底'(错别字)/'到底'等可绕过项,保留真悬念词
      const hookSignals = ['？', '...', '……', '不料', '未完', '究竟', '难道', '谁知', '何曾', '岂料', '忽见', '此刻'];
      const hasSignal = hookSignals.some(s => tail200.includes(s));
      // 增补悬念结构检测:章末 200 字含疑问句(？)或反转信号(但/却/不料+后接新信息)
      const hasReversal = /(但|却|不料)[^，。！？\n]{2,30}/.test(tail200);
    ...
    try {
      ...
      if (match) {
        ...
        if (typeof parsed.score === 'number') {
          ...
          // 门槛从 >8 下调到 >4(5-8 hits 也压缩高分,与 issue push 门槛一致)
          const finalScore = aiTasteHits > 4 ? Math.min(parsed.score, aiTasteScore) : parsed.score;
...
// 阈值依据：单次调用 maxTokens cap 32000，每章 JSON ≈ 200-340 tokens（含Σ契约注解），
//   32000/340 ≈ 94 章，留余量取 80。>80 章走分卷路径，每卷 20 章 maxTokens 8192 足够。
//   原 200 阈值导致 200 章走单次分支，cap 32000 仅够 ~106 章 → 90% 校验失败 → 重试 3 次耗尽 failed。
const OUTLINE_SINGLE_CALL_MAX_CHAPTERS = 80;
...
${volIndex === 0 ? '- 【黄金三章】第1章大纲必须 800 字内触发主线冲突且前 300 字最后一句反转;第2章金手指"做出来";第3章锁定可量化短期目标\n' : ''}- 【矛盾网三层】全书保持 2-3 条矛盾线同时运行(章级 2-3 章解决/卷级卷末解决/书级大结局解决),每次解决一个矛盾必须激活或加深另一个
- 只输出 JSON，不要其他文字`;
...
export function checkPositioningDistribution(items: ParsedOutlineItem[]): {
  ...
  // 风险3修复: 连续run检测 —— 高压/低压连续>2章告警（防节奏疲劳）
  //   原: 只校验全局占比,无连续 run 检测。LLM 可能输出高压×6（全局占比合规但局部连续 6 章高压疲劳）
  //   现: 遍历 items 统计每类最大连续章数,高压/低压连续>2 即告警（daemon.ts 打 task_log warn 可见）
  let maxRun: { 'high-pressure': number; 'low-pressure': number } = { 'high-pressure': 0, 'low-pressure': 0 };
  let curRun: { 'high-pressure': number; 'low-pressure': number } = { 'high-pressure': 0, 'low-pressure': 0 };
  let prevPos: string | null = null;
  for (const item of items) {
    const pos = item.positioning;
    if (pos === 'high-pressure' || pos === 'low-pressure') {
      if (pos === prevPos) curRun[pos]++;
      else curRun[pos] = 1;
      maxRun[pos] = Math.max(maxRun[pos], curRun[pos]);
    } else {
      curRun = { 'high-pressure': 0, 'low-pressure': 0 };
    }
    prevPos = pos ?? null;
  }
  if (maxRun['high-pressure'] > 2) warnings.push(`高压章连续 ${maxRun['high-pressure']} 章，超过2章上限，易致读者疲劳，建议打散`);
  if (maxRun['low-pressure'] > 2) warnings.push(`低压章连续 ${maxRun['low-pressure']} 章，超过2章上限，节奏拖沓，建议插入推进章`);
